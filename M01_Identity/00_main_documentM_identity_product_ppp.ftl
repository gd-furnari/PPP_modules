...
<#--Documents M - Section 1 (Identity) for the mixture/product.-->
...
<#import "common_module_product_basic_information.ftl" as keyProd>
...
<#if _subject.submissionType?has_content>
    <#if _subject.submissionType?matches(".*MICRO.*", "i")>
        ...
        <#assign docName="MP"/>
        <#assign docFullName="Identity of the plant protection product"/>

    <#elseif _subject.submissionType?matches(".*MAXIM.*", "i") >
        <#assign workingContext="MRL"/>
        <#assign docName="MRL"/>
        <#assign docFullName="Identity of the active substance"/>
...
<#--    &lt;#&ndash; Annex for reference substances (uses composition and basic information common modules)&ndash;&gt;-->
    <chapter label="Annex">
        <title role="HEAD-1">Information on Reference Substances</title>

        <#list keyProd.referenceSubstancesInformation as refSub>
            <sect4 role="NotInToc" xml:id="${refSub.documentKey.uuid!}">
                ...
                <#if workingContext=="MICRO"><#assign refSubInfo=refSubInfo?replace("IUPAC", "IUPAC or scientific")/></#if>
                ${refSubInfo}
            </sect4>
